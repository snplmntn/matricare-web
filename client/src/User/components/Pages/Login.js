...
export default function Login() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      const userEmail = response.data.email; // Assume the email is returned from the backend
      ...
      setError(err.response ? err.response.data.message : 'Login error. Please try again.');
      ...
      const handleResendEmail = async () => {
        try {
          await axios.post('http://localhost:5005/users/resend-email', { email });
      ...
      <div className="container-login">
        ...
        >
         <div className="email-icon"><MdMarkEmailRead /></div>
          ...
          <p>You're almost there! We've sent a verification email to <strong>{email}</strong>. <br></br>You need to verify your email address to log into MatriCare.</p>
